...
TMINPR<-28.64 # degrees C, voluntary thermal minimum (lower body temperature for foraging) # based on Yuni's field data for lowland site across all seasons. This wa the lowest preferred (in Autumn)
TBASK<-18.1#5 # degrees C, minimum basking temperature 
TEMERGE<-10.3 # degrees C, temperature at which animal will move to a basking site, educated guess
ctmax<-41.78  # degrees C, critical thermal maximum (animal will die if ctkill = 1 and this threshold is exceeded). Using same data (Orford pop data)
ctmin<-4.27 # degrees C, critical thermal minimum (used by program to determine depth selected when inactive and burrowing) # Mandy's data Using same data (Orford pop data)
...
TPREF<-30.395 # preferred body temperature (animal will attempt to regulate as close to this value as possible) # mean of Yuni's Orford data across all seasons
...
debpars<-as.data.frame(read.csv('DEB models/Niveoscincus_ocellatus_highland/DEB_pars_Niveoscincus_ocellatus_highland.csv',header=FALSE))$V1
fract<-1.22
f<-1.
...
clutchsize<-9. # clutch size, if using regression below, make this the max clutch size
...
daylengthstart<- 12.5 # threshold daylength for initiating breeding: highland vitellogenises begins end of Oct
daylengthfinish<- 12.5 # threshold daylength for terminating breeding. Latest births start of march
...
plot(debout$SVL~debout$dates,type='l',ylim=c(20,95)) # predicted SVL vs time
...
subdate<-subset(environ, format(environ$dates,"%y/%m")=="01/05") # use this to subset a particular year and month
#subdate<-environ # just use the whole data set
with(subdate, plot(TC~dates,ylim=c(-20,50),type = "l",col='blue')) # plot Tb
...
TA<-10191 # Arrhenius temperature
