...
TMAXPR<-35.79 # degrees C, voluntary thermal maximum (upper body temperature for foraging and also burrow depth selection) # based on Yuni's field data for lowland site across all seasons
TMINPR<-19.61 # degrees C, voluntary thermal minimum (lower body temperature for foraging) # based on Yuni's field data for lowland site across all seasons
TBASK<-19.61#5 # degrees C, minimum basking temperature 
TEMERGE<-4 # degrees C, temperature at which animal will move to a basking site, educated guess by Caldwell. Ocellatus value 10, but microlepidotus is more cold adapted with ctmin lower by approx 5 deg.
ctmax<-40.55  # degrees C, critical thermal maximum (animal will die if ctkill = 1 and this threshold is exceeded)
ctmin<-0.79 # degrees C, critical thermal minimum (used by program to determine depth selected when inactive and burrowing) # Mandy's data
...
TPREF<-28.41 # preferred body temperature (animal will attempt to regulate as close to this value as possible) # mean of Yuni's Orford data across all seasons
...
debpars<-as.data.frame(read.csv('DEB models/Niveoscincus_microlepidotus/DEB_pars_Niveoscincus_microlepidotus.csv',header=FALSE))$V1
fract<-1
f<-1.
...
clutchsize<-2. # clutch size, if using regression below, make this the max clutch size
clutch_ab<-c(2.1,11.2) # paramters for relationship between length and clutch size: clutch size = a*SVL-b, make zero if fixed clutch size
...
plot(debout$CUMREPRO/1000~debout$dates,type='l',col='red', ylab='total energy, kJ',xlab="date") # plot energy in the reproduction buffer (captial for egg production, but not currently transformed to eggs)
points(debout$CUMBATCH/1000~debout$dates,type='l') # plot energy in batch buffer (yolking eggs)
abline(v=year_vals$dates,col='grey',lty=2) # add lines to show beginning of each year
...
subdate<-subset(environ, format(environ$dates,"%y")=="01") # use this to subset a particular year and month
...
with(shadmet, points(SNOWDEP~dates,type = "l",col="light blue")) # plot reference (1.5m) air temp
