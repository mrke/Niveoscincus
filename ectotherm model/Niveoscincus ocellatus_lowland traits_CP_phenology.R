...
# MAXSHADES<-as.matrix(rep(80,length(rainfall)))
# elev<-as.numeric(1000)
# REFL<-as.numeric(0.2)
# longlat<-as.matrix(longlat)
# ectoin<-as.matrix(rbind(elev,REFL,longlat,0,0,1990,1990+nyears-1))
# wetlandTemps=matrix(data = 0., nrow = nrow(metout), ncol = 1)
# wetlandDepths=matrix(data = 0., nrow = nrow(metout), ncol = 1)
# RAINFALL<-rainfall
