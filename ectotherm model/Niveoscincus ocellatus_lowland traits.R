...
TMAXPR<-34.7 # degrees C, voluntary thermal maximum (upper body temperature for foraging and also burrow depth selection) # based on Yuni's field data for lowland site across all seasons
...
TBASK<-23 #5 # degrees C, minimum basking temperature
TEMERGE<-10 # degrees C, temperature at which animal will move to a basking site, Yunis data
...
maxdepth<-5 # maximum depth (soil node) to which animal can retreat if burrowing
...
minclutch<-1 # if this is zero, will delay reproduction until it reaches clutch size as specified above, othewise will have a clutch if it gets to this minimum value in a given repro season
...
Growth<-read.csv('life history/N_ocellatus_growth.csv') # read in first dataset
Growth<-subset(Growth,site=='Orford' ) # choose site 
#Growth<-subset(Growth,site=='CP' ) # choose site 
Growth$date<-as.POSIXct(Growth$date,format='%d/%m/%Y') # convert dates
points(Growth$SVL~Growth$date,col='red') # plot results

Growth_skeleto<-read.csv('life history/N_ocellatus_growth_skeleto.csv') # bring in second dataset
...
days<-rep(seq(1,nyears,1),365*24)
days<-as.data.frame(days[order(days)])
days<-metout$JULDAY+365*(days-1)

# plot activity windows
plotenviron<-subset(environ,YEAR%in%1:5) #choose time period
#plotenviron<-environ
forage<-subset(plotenviron,ACT==2) # get foraging times
bask<-subset(plotenviron,ACT==1) # get basking times
night<-cbind(metout,days)
colnames(night)[20]<-'days'
night<-subset(night,ZEN==90) # get night period
with(bask,plot((TIME-1)~DAY,pch=15,cex=.5,col='grey',ylab='hour of day',ylim=c(0,23),xlab='day of year')) # plot basking
with(forage,points((TIME-1)~DAY,pch=15,cex=.5,col='blue')) # plot foraging
with(night,points(TIME/60~days,pch=15,cex=.5)) # plot night hours
...
subdate<-subset(environ, format(environ$dates,"%y/%m")=="01/01") # use this to subset a particular year and month
...
#with(subdate, points(DEP/10~dates,type = "l",col="brown")) # plot depth under ground
...
abline(ctmax,0,lty=1,col='red',lwd=2)
...
#abline(TEMERGE,0,lty=2,col='yellow',lwd=2)
...
yearsout #check reproduction
debout
environ
head(environ)

# Get hibernation dates
March<-subset(environ, format(environ$dates,"%m")=="03") # use this to subset a particular year and month
ActiveMarch<-subset(March,ACT>0) # use this to subset a particular year and month
April<-subset(environ, format(environ$dates,"%m")=="04") # use this to subset a particular year and month
ActiveApril<-subset(April,ACT>0) # use this to subset a particular year and month
ActiveApril2006<-subset(ActiveApril, format(ActiveApril$dates,"%y")=="06")
May<-subset(environ, format(environ$dates,"%m")=="05") # use this to subset a particular year and month
ActiveMay<-subset(May,ACT>0) # use this to subset a particular year and month
June<-subset(environ, format(environ$dates,"%m")=="06") # use this to subset a particular year and month
ActiveJune<-subset(June,ACT>0) # use this to subset a particular year and month
July<-subset(environ, format(environ$dates,"%m")=="07") # use this to subset a particular year and month
ActiveJuly<-subset(July,ACT>0) # use this to subset a particular year and month
August<-subset(environ, format(environ$dates,"%m")=="08") # use this to subset a particular year and month
ActiveAugust<-subset(August,ACT>0) # use this to subset a particular year and month
September<-subset(environ, format(environ$dates,"%m")=="09") # use this to subset a particular year and month
ActiveSeptember<-subset(September,ACT>0) # use this to subset a particular year and month
