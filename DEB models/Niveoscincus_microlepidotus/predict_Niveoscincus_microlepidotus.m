...
%%
function [Prd_data, info] = predict_Niveoscincus_microlepidotus(par, chem, T_ref, data)
  ...
  % life span
  pars_tm = [g; l_T; (h_a*zm^2)/ k_M^2; s_G];  % compose parameter vector at T_ref
  ...
  L = (4.16*d_V / d_V/ (1 + f * w)).^(1/3); % cm, structural length, use dry mass for 4.16 g wet mass
  ...
  EO = (- 1 * (JM(:,3) * X_gas) .* tempcorr(273+TO(:,1), T_ref, pars_T))/4.16/24*1000;
